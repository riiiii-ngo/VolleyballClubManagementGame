...
function renderLoginScreen() {
  ...
  app.innerHTML = `
    <div class="login-screen">
      <div class="login-card">
        ...
        <button id="btn-google-login" class="btn-google btn-large btn-full">
  ...
  document.getElementById('btn-google-login').addEventListener('click', async () => {
    clearError();
    showLoading('Googleログイン中...');
    window._oauthLoginPending = true;
    try {
...
function renderPlayerList(state) {
  ...
  state.players
    ...
    .forEach(p => {
      ...
      const starterSlot = Object.entries(state.starters).find(([, v]) => v === p.id)?.[0] ?? null;
      ...
      rosterHtml += `<tr class="${isStarter ? 'starter-row' : ''}" style="${p.isInjured ? 'opacity:0.45;' : ''}">
        <td>${posDisplay} ${p.name}${starterSlot ? ` <span class="badge-st">${starterSlot}</span>` : ''}${injuryBadge}</td>
        <td>${selectHtml}</td>
        <td>${p.grade}年</td>
  ...
  const rows = sortedPlayers.map(p => {
    ...
    return `
      <tr class="${isStarter ? 'starter-row' : ''}" style="${p.isInjured ? 'opacity:0.4' : ''}">
        <td>${p.isAllRounder ? '全' : p.position}</td>
        <td>${p.name}${starterSlot ? ` <span class="badge-st">${starterSlot}</span>` : ''}</td>
        <td>${p.grade}年</td>
        <td><strong>${playerOverall(p)}</strong></td>
